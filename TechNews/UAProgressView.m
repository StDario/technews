...
- (void)addFill {
	if (self.fillOnTouch) {
        ...
        ((UILabel *)self.centralView).textColor = [UIColor whiteColor];
        
        CABasicAnimation *highlightAnimation = [CABasicAnimation animationWithKeyPath:@"backgroundColor"];
        highlightAnimation.fromValue           = (id)[UIColor clearColor].CGColor;
        highlightAnimation.toValue             = (id)[self tintColor].CGColor;
        highlightAnimation.removedOnCompletion = NO;
        highlightAnimation.duration = 0.8;
...
- (void)removeFillAnimated:(BOOL)animated {
	if (self.fillOnTouch) {
        ...
        ((UILabel *)self.centralView).textColor = [self tintColor];
